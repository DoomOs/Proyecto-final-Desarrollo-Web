...
class ItemDescriptionForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Fieldset('',
                ...
                'description',
                HTML("""<h5>Detalles</h5>"""),
                Div(HTML('<label class="col-form-label">Fecha de vencimiento</label>'), 'due_date', css_class='form-row'),
            ),)
    ...
    description = forms.CharField(widget=forms.Textarea(attrs={'style': 'border-style: none; resize: none; background-color: white !important; width:90%; color: black; max-height:50vh;', 
                                                               ...
                                                               'placeholder': 'Ingresa una descripcion',}), label='', required=False)

    due_date = forms.DateField(widget=forms.DateInput(format=('%Y-%m-%d'), attrs={'placeholder':'Selecciona una fecha', 'type':'date'}), label='', required=False)